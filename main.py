...
def ObjectAnalysis(photo):
    global value,objectBtn, count
    ...
    """
    if photoTemp not in fileCarousel:
        photoTemp="R2.jpg"
    """
    """
    ...
    detections = detector.detectObjectsFromImage(input_image=os.path.join(execution_path , photo), output_image_path=os.path.join(execution_path , "imagenew.jpg"))
    ...
    if count != 0:
        ...
        value = tk.Label(object, text= "your buisness has..."+str(count)+"...value!", font = ("Times New Roman", 20), borderwidth=5, relief="ridge").pack(pady=15)
...
title = tk.Label(window, text="H2OOPS: Splashing Into Flood Coverage!", font = ("Times New Roman", 30)).pack(pady=7.5)

def nested():
    ObjectAnalysis(photoEntry.get())

def photoInput():
    global photoEntry, ActionBtn
    photoEntry = tk.Entry(object, width = 30, bg = "#CBC5EA", fg ="black", borderwidth = 2)
    photoEntry.pack(pady=15)
    ActionBtn = tk.Button(object, text="Enter Photo", command = nested, relief="ridge").pack(pady = 10)
    

    

def userInput():
    global nearestPlaces, option1, option2, option3, option4, option5, result, chances
    ...
    result = tk.Label(proximity, text= "Your Liklihood of Flooding by Region Is: " + str(round(chances*100, 2)) + "%", font = ("Times New Roman", 20), fg="#73628A", bg = "#CBC5EA", borderwidth=5, relief="ridge").pack(pady=30)
...
def proximityOpen():
  ...
  gameLabel1 = tk.Label(proximity, text="Find out your liability towards flooding!\nPlease input your address below:", font = ("Times New Roman", 32), bg = '#313D5A', borderwidth=5, relief="ridge").pack(pady=15)
...
def objectOpen():
  global object, objectLabel, objectBtn, objectActionBtn, photoEntry, photo
  object = tk.Toplevel(bg ="#EAEAEA")
  object.title("Object Recognition Screen")
  object.geometry("800x800")
  objectLabel = tk.Label(object, text="Find out your assets' liability against flooding!", font = ("Times New Roman", 30), bg = '#313D5A', borderwidth=5, relief="ridge").pack(pady=15)
  
  objectActionBtn = tk.Button(object, text="Activate Recognizer", command = photoInput, relief="ridge").pack(pady = 10)
  
  

def resultsOpen():
  global results, resultsLabel, resultsBtn, resultGo, percent, resultGo2
  results = tk.Toplevel(bg ="#EAEAEA")
  results.title("Reccomentation Screen")
  results.geometry("800x800")
  resultsLabel = tk.Label(results, text="View your recommended insurance plan!", font = ("Times New Roman", 30), bg = '#313D5A', borderwidth=5, relief="ridge").pack(pady=15)
  percent = round(count/20 * chances * 100, 2)
  resultGo = tk.Label(results, text= "Your Calculated Risk Is: " + str(percent) + "%", font = ("Times New Roman", 20), fg="#73628A", bg = "#CBC5EA", borderwidth=5, relief="ridge").pack(pady=30)
  if(percent<15):
      resultGo2 = tk.Label(results, text= "Your Recommended Plan in the BRONZE PLAN", font = ("Times New Roman", 20), fg="#73628A", bg = "#CBC5EA", borderwidth=5, relief="ridge").pack(pady=30)
  if(percent>=15 and percent <=40):
      resultGo2 = tk.Label(results, text= "Your Recommended Plan in the SILVER PLAN", font = ("Times New Roman", 20), fg="#73628A", bg = "#CBC5EA", borderwidth=5, relief="ridge").pack(pady=30)
  if(percent>=40):
      resultGo2 = tk.Label(results, text= "Your Recommended Plan in the GOLD PLAN", font = ("Times New Roman", 20), fg="#73628A", bg = "#CBC5EA", borderwidth=5, relief="ridge").pack(pady=30)
  resultsBtn = tk.Button(results, text="Close Window", command = results.destroy, relief="ridge").pack()
  
floodIMG = tk.PhotoImage(file = 'D:\\HackUTD\\couldWork\\flood2.png', width = 525, height = 485)
...
infoB = tk.Button(frame, text = "Flood Proximity", font = ("Times New Roman", 10), height= 8, width=22, command = proximityOpen)
quizB = tk.Button(frame, text = "Object Recognition", font = ("Times New Roman", 10), height= 8, width=22, command = objectOpen)
gameB = tk.Button(frame, text = "Recommended Insurance", font = ("Times New Roman", 10), height= 8, width=22, command  =resultsOpen)
